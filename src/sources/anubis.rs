...
/// Parse the Anubis (jldc.me) response, a flat JSON array of subdomain strings:
/// `["a.example.com", "b.example.com"]`. Malformed bodies yield an empty list.
pub fn parse_anubis(body: &str) -> Vec<String> {
...
mod tests {
    ...
    #[test]
    fn empty_on_garbage() {
        assert!(parse_anubis("not json").is_empty());
        assert!(parse_anubis("{}").is_empty());
    }
}
